...
import type { QrCil, QrKod, QrStatistika } from './qr-types'
...
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://propojo.cz'
...
export async function getQrCile(): Promise<QrCil[]> {
  ...
  const cile: QrCil[] = (karty ?? []).map(k => ({
    ...
    itemId: null,
    nazev: k.title,
    popis: k.city ? `Rovnou na ceník a volné termíny · ${k.city}` : 'Rovnou na ceník a volné termíny',
  }))
  ...
  cile.push({
    ...
    itemId: null,
    nazev: 'Můj profil',
    popis: 'Všechny nabídky a hodnocení pohromadě — spíš na vizitku než na dveře',
  })
...
export async function getQrStatistika(qrCodeId: string): Promise<QrStatistika> {
  ...
  return {
    ...
    tydne: (scans ?? []).length,
    poDnech: Object.entries(dny).map(([den, pocet]) => ({ den, pocet })),
  }
}
